...
public class DbConnect {
    ...
    public static Connection getConnection(){
        try{
            ...
            conn = DriverManager.getConnection("jdbc:mysql://localhost:3306/easyhealth", "root", "");
        }catch (Exception e){
            e.printStackTrace();
        }
        return conn;
        ...
        try{
            ...
            while(rs.next()) {
                productList.add(new Product(
                        ...
                        Float.parseFloat(rs.getString("calory"))
                ));
            }
        }catch (Exception e){
            e.printStackTrace();
    ...
    public float getCaloryProduct(int id){
        try{
            var result = getConnection().prepareStatement("SELECT * FROM products").executeQuery();
            while(result.next()){
                if (result.getInt(1)== id){
                    return result.getFloat("calory");
                }
            }
        }catch(Exception e){
            throw new Error("Not working! " + e);
