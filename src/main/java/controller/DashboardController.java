...
public class DashboardController implements Initializable {
    ...
    void btnCreate(ActionEvent event) {
        ...
        float calory = Float.parseFloat(ttCalory.getText());

        if(btnCreate.getText().equals("Create")){
            try{
                connect.insertQuery("products", "(id_category, name, image, price,calory)", "("+category+", '"+ name+ "', '"+image+ "', "+ price + " , "+calory+ ")");
            ...
            tableView.setRowFactory(tv -> {
                ...
                row.setOnMouseClicked(event -> {
                    if (! row.isEmpty() && event.getButton()== MouseButton.PRIMARY
                        ...
                        ttCalory.setText(""+clickedRow.getCalory());
...
//                        showImageClicked();
    ...
    @FXML
    public TableColumn<OrderDetail, Float> OrderCaloryCl;
    @FXML
    ...
    private void showOrderItemTableView() {
        ...
        OrderCaloryCl.setCellValueFactory(new PropertyValueFactory<>("calory"));
    ...
    public void addItem(ActionEvent event) { // Add item to order detail
        ...
        conn.insertQuery("orderdetails", "(`id_product`, `id_order`, `quantity`, `price`, `calory`)", "("+id_prd+", "+id_order
                +", "+qtyTxt +" , '"+conn.getPriceProduct(id_prd)+"' , '"+conn.getCaloryProduct(id_prd)+"')");
